...
3. **Build the site assets** (requires ffmpeg; install it with `winget install --id Gyan.FFmpeg -e`):

         node tools/process-clips.mjs build

4. Reload the site. Background color, hero frames, posters, and reaction clips
   all come from `public/assets/frames/meta.json`, so no code changes are needed.
